...
- **HTTP (Standard Web):** Like sending a **Letter**.
  ...
  5.  **The connection ends.** If you want to say something else, you must write a _new_ letter.
...
1.  **Express (HTTP):** Used for setup and control.
    - _Analogy:_ The receptionist who checks you in.
2.  **WS (WebSocket):** Used for the actual chatting.
    - _Analogy:_ The conversation you have after checking in.
...
3.  **Broadcast:** The Server hears this and repeats it to everyone else in the room.
    - _Code Logic:_ Loop through the `clients` list and send the message to each one.
...
- _Example:_ The test checks for a message _before_ it arrives from the server.
- _Fix:_ I used Playwright's `await expect(...)` which automatically waits and retries until the message appears.
